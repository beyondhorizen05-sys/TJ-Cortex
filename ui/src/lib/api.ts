...
export const api = {
  ...
  createAgent: (input: Partial<Agent> & { name: string }) =>
    http<Agent>('/agents', { method: 'POST', body: JSON.stringify(input) }),
  updateAgent: (id: string, patch: Partial<Agent>) =>
    http<Agent>(`/agents/${id}`, { method: 'PATCH', body: JSON.stringify(patch) }),
  ...
  runTurn: (body: { agentId: string; conversationId?: string; input: string }) =>
    http<{ conversationId: string; text: string; reasoning?: string }>('/run', { method: 'POST', body: JSON.stringify(body) }),
  ...
  setProviderKey: (id: string, apiKey: string) =>
    http<{ ok: true }>(`/providers/${id}/key`, { method: 'POST', body: JSON.stringify({ apiKey }) }),
  deleteProviderKey: (id: string) => http<{ ok: true }>(`/providers/${id}/key`, { method: 'DELETE' }),
  setProviderBaseUrl: (id: string, baseUrl: string) =>
    http<{ ok: true }>(`/providers/${id}/base-url`, { method: 'POST', body: JSON.stringify({ baseUrl }) }),
  providerHealth: (id: string) =>
    http<{ ok: boolean; error?: string; accountEmail?: string }>(`/providers/${id}/health`, { method: 'POST' }),
  ...
  googleSetClient: (clientId: string, clientSecret?: string) =>
    http<{ ok: true }>('/auth/google/client', { method: 'POST', body: JSON.stringify({ clientId, clientSecret }) }),

  // Permissions
  listPolicies: () => http<any[]>('/permissions/policies'),
  resolvePermission: (id: string, decision: 'allow_once' | 'allow_always' | 'deny') =>
    http<{ ok: true }>('/permissions/resolve', { method: 'POST', body: JSON.stringify({ id, decision }) }),

  // Tools
  listTools: () => http<ToolDescriptor[]>('/tools'),
  invokeTool: (agentId: string, toolName: string, args: Record<string, unknown>) =>
    http<any>('/tools/invoke', { method: 'POST', body: JSON.stringify({ agentId, toolName, args }) }),

  // Recipes / Skills
  listRecipes: () => http<Recipe[]>('/recipes'),
  createRecipe: (input: Partial<Recipe> & { name: string }) =>
    http<Recipe>('/recipes', { method: 'POST', body: JSON.stringify(input) }),
  deleteRecipe: (id: string) => http<{ ok: true }>(`/recipes/${id}`, { method: 'DELETE' }),

  listSkills: () => http<Skill[]>('/skills'),
  createSkill: (input: Partial<Skill> & { name: string; kind: Skill['kind'] }) =>
    http<Skill>('/skills', { method: 'POST', body: JSON.stringify(input) }),
  deleteSkill: (id: string) => http<{ ok: true }>(`/skills/${id}`, { method: 'DELETE' }),

  // MCP
  listMcp: () => http<McpServerConfig[]>('/mcp'),
  listMcpLive: () => http<any[]>('/mcp/live'),
  createMcp: (input: Partial<McpServerConfig> & { name: string; transport: 'stdio' | 'sse' }) =>
    http<any>('/mcp', { method: 'POST', body: JSON.stringify(input) }),
  ...
  setSetting: (key: string, value: unknown) =>
    http<{ ok: true }>(`/settings/${key}`, { method: 'PUT', body: JSON.stringify(value) }),
};